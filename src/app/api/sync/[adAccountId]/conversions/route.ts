/**
 * POST /api/sync/[adAccountId]/conversions
 *
 * Per-account custom conversions sync. Mirrors the other per-account routes.
 */
...
export async function POST(
  ...
  try {
    const result = await syncConversionsForAccount(row.id);

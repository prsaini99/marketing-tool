...
`/dashboard/studio` ("Ad Studio") is the standalone entry point to `generate-ad-image.ts`, with a brand kit (`BrandKit`/`BrandAsset` — colour palette, brand name, tagline, theme notes, a do-not list, one logo, several style references, stored the same way captured Meta assets are, under `src/lib/storage/assets.ts`) and a per-request image-model picker (`gpt-image-2` / `gpt-image-1.5` / `gpt-image-1` / `chatgpt-image-latest`) that overrides the module's `DEFAULT_MODEL` for both generate and tweak.

The studio's primary input is a **format**, not a brief. `src/server/services/ai/ad-formats.ts` is a pure, import-free catalogue of seventeen static ad layouts, each carrying its frame instruction, the copy slots the image must render, the schematic geometry, and a `needs` of `none` / `product` / `proof`. `needs` is enforced role-aware on both sides: a `product` format requires a reference tagged `product`, a `proof` format one tagged `proof`, and the generate route is the enforcement boundary. Formats that rest on real evidence (before/after, quote card, review stack, founder quote) are never synthesised.

Copy is written **before** the image by a separate cheap text call (`ad-copy.ts`, gpt-4o-mini, bounded by its own 15s timeout since `completeJson` takes no `AbortSignal`), so the exact on-image strings can be quoted into the prompt — the documented fix for garbled text — and so an empty brief still produces a specific ad. That stage **may not invent facts**. `copy-guard.ts` (also import-free) extracts every figure from the generated copy and rejects any that is absent from the brief and brand kit, by exact set membership, the same discipline `automation/ai-guards.ts` uses for prices. A four-digit number is exempted as a year only when a year cue sits beside it (`since 2019`, `© 2024`, `2019-2024`), never when a currency or discount marker does — a bare range test let the whole Indian price band (₹1,999) and "1,999 reviews" through unflagged. A failing figure triggers one retry, then the slot is dropped and reported to the operator as `droppedSlots`.

`generate-ad-image.ts` composes the prompt from separable frames: `CRAFT_FRAME` (always), `PROMO_FRAME` (the classic sale-ad layout — suppressed by `promoFrame: false`, which the studio's format path passes), `PRODUCT_FRAME` (only when a `product`-role reference exists, and holding only what is true of any product ad), then `OUTPUT_FRAME`. Framing and anti-AI-trope rules are stated once, in `CRAFT_FRAME`.

**`BrandKit.businessId` is nullable, and NULL is meaningful**: it is the workspace's own kit — the operator's brand, the one edited when the topbar switcher says "All clients". A partial unique index (`BrandKit_workspace_singleton`) keeps there being exactly one, since Postgres lets NULLs repeat under a plain unique constraint. **Nothing inherits between scopes**: a client with no kit gets an empty one, never the workspace's, because each client owns their brand outright and is expected to edit it behind their own login once multi-user lands — a client session scoped to its own `businessId` can never address the NULL row. Every function in `src/server/services/brand/kit.ts` therefore takes `KitScope = string | null` rather than a bare string, and the routes fold an absent or empty `client`/`businessId` to the workspace scope. Workspace assets live under `brand/_workspace/<assetId>`.

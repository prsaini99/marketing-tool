...
- `HIGGSFIELD_KEY_ID` / `HIGGSFIELD_KEY_SECRET` — required for video generation in the Ad Studio; without both, `higgsfieldConfigured()` returns false and the video panel is disabled rather than broken. Video is generated by Higgsfield's Kling v2.1 Master model through `src/lib/higgsfield/`, which is the only module permitted to call Higgsfield — the same rule as `src/lib/meta/`.
